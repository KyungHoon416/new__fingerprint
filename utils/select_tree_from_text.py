...
def select_tree_from_metrics(radial, texture_std, ridge_mean, avg_angle):
    ...
    if center_density > 0.08 and texture_std > 12 and abs(avg_angle) < 10:
        score["느티나무"] += 1.0
    if texture_std < 10 and center_density < 0.05 and ridge_mean < 0.05:
        score["자작나무"] += 1.0
    if avg_angle > 15 and ridge_mean > 0.07:
        score["소나무"] += 1.0
...
def hybrid_select_tree(thumb_text, index_text, radial, texture_std, ridge_mean, avg_angle, weight_text=0.6, weight_metric=0.4):
    ...
    metric_tree, metric_score = select_tree_from_metrics(radial, texture_std, ridge_mean, avg_angle)
    ...
    # 모든 점수가 0일 경우 fallback
    best_tree = max(score, key=score.get, default="느티나무")

    return tree_profiles.get(best_tree, {
        "name": "느티나무",
